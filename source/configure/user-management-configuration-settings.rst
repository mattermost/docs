...
.. important::
  ...
  - **Slash commands** will continue to work after user deactivation. Consider deleting the existing slash command token and creating a new slash command associated with a different user account to decouple sensitive token data from the deactivated user account. In addition, consider regenerating the token and check that the deactivated user doesn't have access to the slash command **Request URL** which is the callback URL to receive the HTTP POST or GET event request when the slash command is run.
