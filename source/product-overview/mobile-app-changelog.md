...
**Note:** Mattermost Mobile App v2.33.1 contains a medium level security fix. Updating is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the Mattermost Responsible Disclosure Policy.

### Bug Fixes
 - Fixed an issue where some users experienced crashes when opening certain channel types.
 - Fixed a performance issue when loading large message threads.
 - Fixed an issue with push notifications not displaying properly on iOS devices.
 - Fixed a memory leak that could occur during extended app usage.
