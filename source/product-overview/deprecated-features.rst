...
- Omnibus support will be deprecated. The last ``mattermost-omnibus`` release will be v10.12. System administrators using Omnibus deployments should plan to migrate to alternative deployment methods before upgrading to Mattermost v11. See the :doc:`deployment guide </deployment-guide/server/server-deployment-planning>` for recommended alternatives including Kubernetes, Linux installations, and container-based deployments.
