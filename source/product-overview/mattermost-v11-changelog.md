...
- **11.9.1, released 2026-08-31**
  - Mattermost v11.9.1 contains medium severity level security fixes. [Upgrading](https://docs.mattermost.com/upgrade/upgrading-mattermost-server.html) to this release is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the [Mattermost Responsible Disclosure Policy](https://mattermost.com/security-vulnerability-report/).
  ...
  - Fixed an issue where the data retention policy teams endpoint returned unsanitized team data, exposing the invite_id and email of private teams to users who only held read-only Data Retention permission. Thanks to 0x7oda7123 for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where OAuth Dynamic Client Registration redirect URI allowlist validation could be bypassed using a crafted query string, potentially allowing an attacker-controlled redirect URI to be registered. Thanks to aryangurjarf401a3 for contributing to this improvement under the Mattermost responsible disclosure policy.
...
- **11.8.5, released 2026-07-31**
  - Mattermost v11.8.5 contains medium severity level security fixes. [Upgrading](https://docs.mattermost.com/upgrade/upgrading-mattermost-server.html) to this release is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the [Mattermost Responsible Disclosure Policy](https://mattermost.com/security-vulnerability-report/).
  ...
  - Pre-packaged Microsoft Calendar plugin version [v1.6.2](https://github.com/mattermost/mattermost-plugin-mscalendar/releases/tag/v1.6.2).
  - Fixed an issue where the data retention policy teams endpoint returned unsanitized team data, exposing the invite_id and email of private teams to users who only held read-only Data Retention permission. Thanks to 0x7oda7123 for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed a case where a specially crafted message containing a URL followed by many trailing characters could cause quadratic-time parsing, allowing an authenticated user to degrade server performance for other users. Thanks to Adam Korczynski for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where OAuth Dynamic Client Registration redirect URI allowlist validation could be bypassed using a crafted query string, potentially allowing an attacker-controlled redirect URI to be registered. Thanks to aryangurjarf401a3 for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where uploading many files hard to process could grow the number of goroutines spawned, and also could stop other files from being parsed and indexed. Thanks to JG Heithcock for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where a channel admin could remove a system-admin-assigned parent access control policy from a channel via the general policy update endpoint. Thanks to kiwi_71 for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where uploading a specially crafted TIFF image as a profile picture, file attachment, team icon, or brand image could cause unbounded memory allocation on the server. Thanks to alimursaliyev for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where a malformed Microsoft Graph webhook payload containing a null entry in the value array could crash the Microsoft Calendar plugin via a nil pointer dereference, taking the integration offline for all users until the plugin restarted. Thanks to kamil-sawicki for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where a malformed interactive message (post-action) request could crash the Microsoft Calendar plugin process and make all of its routes unavailable, because handler panics were not recovered and certain request fields were type-asserted without validation. Thanks to alimursaliyev for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where an attacker could send compressed SDP messages that unpack to a large size, allowing a denial-of-service attack by sending many such messages and exhausting memory. Thanks to daw10 for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where channel action updates did not verify the stored action was associated with the requested channel. Thanks to minghseinyuc86595 for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where the Playbooks run property update endpoint did not verify that a property field belonged to the targeted run, allowing an authenticated user to crash the plugin. Thanks to alimursaliyev for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where importing a playbook with a null condition could crash the Playbooks plugin. Thanks to alimursaliyev for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where an authenticated user with editor access to a board could submit a block with a malformed ``fields.properties`` value that crashed the Boards plugin notification worker and caused a denial of service. Thanks to alimursaliyev for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where board archive import did not enforce team board creation permissions, allowing restricted users to create boards via imported archives. Thanks to angelatanasov5ac9f8 for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed two issues in the Boards archive import endpoint where a crafted .boardarchive file could cause excessive memory allocation or bypass the configured file size limit, leading to resource exhaustion. Thanks to Edgar Bellot Micó for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where the board duplication, boards-and-blocks, and archive import endpoints did not enforce the per-type board creation permission (``create_public_channel`` / ``create_private_channel``), allowing users whose board-creation rights had been revoked to still create boards. Thanks to c4b8a133-e345-4bbe-9a44-d1ef0938de51 for contributing to this improvement under the Mattermost responsible disclosure policy.
  - Fixed an issue where summarizing certain Zoom chat transcriptions could crash the Agents plugin due to insufficient line-length validation. Thanks to alimursaliyev for contributing to this improvement under the Mattermost responsible disclosure policy.
...
- **11.7.8, released 2026-07-31**
  - Mattermost v11.7.8 contains medium severity level security fixes. [Upgrading](https://docs.mattermost.com/upgrade/upgrading-mattermost-server.html) to this release is recommended. Details will be posted on our [security updates page](https://mattermost.com/security-updates/) 30 days after release as per the [Mattermost Responsible Disclosure Policy](https://mattermost.com/security-vulnerability-report/).
  - Pre-packaged Boards plugin version [v9.2.7](https://github.com/mattermost/mattermost-plugin-boards/releases/tag/v9.2.7).
  - Pre-packaged Playbooks plugin version [v2.9.4](https://github.com/mattermost/mattermost-plugin-playbooks/releases/tag/v2.9.4).
  - Pre-packaged Calls plugin version [v1.11.6](https://github.com/mattermost/mattermost-plugin-calls/releases/tag/v1.11.6).
  - Pre-packaged Microsoft Calendar plugin version [v1.6.2](https://github.com/mattermost/mattermost-plugin-mscalendar/releases/tag/v1.6.2).
