...
If you're using Mattermost 7.0, open boards using the App Bar. If you're using a later version, see the instructions below
...
After creating categories, you can move your boards to those categories by opening the “…” options menu (...) next to the board and clicking on the “Move To…” option to select the category where you want the board to be moved.

If you delete a category with boards in it, then those boards will return back to the default “Boards” category.
Categories are per-user, so you can organize your boards under categories that make sense to you without impacting the boards or categories for other users. If a board is moved to a custom category, then the board will appear under that category for that specific user only. Other users who are members of the board will still continue to see the board in their own categories.

Note: For users upgrading to Mattermost Boards v7.2 or later: 

If you belonged to a workspace prior to v7.2, then you’ll see that the workspaces have been migrated to custom categories in the sidebar. All boards from a workspace are listed under a category of the same name. Boards from direct messages and group messages appear under the default “Boards” category.
