...
1. Add a SAML test connector app.
  ...
  c. Search for "SAML Test Connector", then select **SAML Test Connector (IdP) w/encrypt**.
...
3. Enter the attribute parameters.
  ...
  *Email* and *Username* attributes are required. For Mattermost servers running version 3.3 and earlier, ``FirstName`` and ``LastName`` attributes are also required.
  ...
  h. Repeat the steps above to add the ``Username`` attribute and any other attributes that you need.
...
4. Copy the SSO information.
  ...
  b. Copy the values in the **Issuer URL** and **SAML 2.0 Endpoint (HTTP)** fields and save them for later use.
...
1. Start the Mattermost server, then sign in to Mattermost as a System Administrator. Go to **System Console > Authentication > SAML**.
