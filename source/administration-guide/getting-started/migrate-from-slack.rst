...
During the import process, we advise to create a new team for importing the Slack workspace data. If merging multiple Slack workspaces into a single team is the desired end-result, we recommend completing the import to separate teams, validating the results, then using :ref:`mmctl <administration-guide/admin-tools/mmctl-command-line-tool:mmctl channel move>` to move channels between teams.
...
Ensure you have the Mattermost command line tool ``mmctl`` installed. This allows you to perform different tasks that communicate to Mattermost's API. You'll also want to :ref:`configure authentication <administration-guide/admin-tools/mmctl-command-line-tool:mmctl auth>` for the tool.
...
* Instructions on how to migrate user authenticatation to LDAP or SAML can be found :ref:`here <administration-guide/admin-tools/mmctl-command-line-tool:mmctl user migrate-auth>`.
