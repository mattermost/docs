...
After the script completes, you'll have ``mattermost-x509.key`` and ``mattermost-x509.crt`` in the current directory. Keep the ``.key`` file secure — it's the private key for your SAML Service Provider identity. For details on verifying the generated certificate or password-protecting the private key, see the :doc:`gencert.sh reference </scripts/generate-certificates/gencert>`.
