...
5. In the **Name** field, enter **Mattermost SAML** then click the **Create** button.
6. In the **Mattermost SAML** enterprise application settings, select **Manage > Users and Groups** to assign users and/or groups to the application **or** select **Manage > Properties** then set **Assignment required?** to **No** then click **Save**.
7. In the **Mattermost SAML** enterprise application settings, select **Manage > Single sign-on** then click **SAML** under **Select a single sign-on method**.
8. Select **Edit** in the **Basic SAML Configuration section** then set the below fields then click **Save**:

	1. **Identity (Entity ID)**: ``https://<your-mattermost-url>``
	2. **Reply URL (Assertion Consumer Service URL)**: ``https://<your-mattermost-url>/login/sso/saml``
	3. **Sign on URL**: ``https://<your-mattermost-url>/login``
...
10. Select **Edit** in the **SAML Certificates** section. Select **Sign SAML response and assertion** for **Signing Option** and **SHA-256** for **Signing Algorithm** then click **Save**.
...
13. Click the ``...`` to the right of the imported certificate details, select **Activate token encryption certificate**, then click **Yes** to activate.
