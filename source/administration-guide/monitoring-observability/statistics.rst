...
.. note::
  ...
  To maximize performance for large Enterprise deployments, statistics for total messages, total hashtag messages, total file messages, messages per day, and activated users with messages per day is configurable by changing the ``MaxUsersForStatistics`` value :ref:`in config.json <administration-guide/configuration-reference/reporting-configuration-settings:maximum users for statistics>`.

For advanced metrics for Enterprise deployments, :doc:`see performance monitoring documentation to learn more </administration-guide/operations-scaling/deploy-prometheus-grafana-for-performance-monitoring>`.
...
Replica DB Conns
    The number of active connections currently on one or more of :ref:`your read replica databases <administration-guide/operations-scaling/high-availability-cluster-based-deployment:database configuration>`.
