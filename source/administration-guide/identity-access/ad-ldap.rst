...
Mattermost offers “Same Sign-On” with Microsoft AD/LDAP (formerly known as Active Directory/LDAP). Enable the same credentials used in on-prem AD/LDAP deployments to be reused in Mattermost, with optional :doc:`multi-factor authentication </administration-guide/identity-access/multi-factor-authentication>`.
...
2. **Configure AD/LDAP by editing ``config.json``**
  ...
  - Edit ``config.json`` to enable AD/LDAP based on the :ref:`AD/LDAP settings documentation <administration-guide/configuration-reference/authentication-configuration-settings:ad/ldap>`. When you log in to Mattermost the first user to log in with valid AD/LDAP credentials will be assigned the system admin role.
...
.. note::
   ...
   If you've made a mistake and lock yourself out of the system somehow, you can set an existing account to system admin using the :ref:`mmctl roles <administration-guide/admin-tools/mmctl-command-line-tool:mmctl roles>` command.
...
2. Navigate to the **Sync Performance** section and configure the **Synchronization Interval (minutes)** to specify how often Mattermost accounts synchronize attributes with AD/LDAP. The default setting is 60 minutes. The profile picture attribute is only synchronized when the user logs in.
  ...
  - To configure AD/LDAP synchronization with SAML sign-in, see the :doc:`SAML documentation </administration-guide/identity-access/authentication-methods/saml-based-sso/sso-saml>`.
...
If the issue persists, try selecting the **Test Filters** button to test that the User Filter is correctly formatted. Refer to this :ref:`document <administration-guide/configuration-reference/authentication-configuration-settings:user filter>` for guidance on setting a correct syntax format.
...
2. **ID Attribute:** Used as the unique identifier in Mattermost. It should be an AD/LDAP attribute with a value that does not change, such as ``ObjectGUID``. If a user's ID attribute changes, it will create a new Mattermost account unassociated with their old one. If you need to change this field after users have already logged in, use the :ref:`mattermost ldap idmigrate mmctl tool <administration-guide/admin-tools/mmctl-command-line-tool:mmctl ldap idmigrate>`.
...
1. Check that the user credentials were entered properly - you should log in with the field set as the :ref:`*ID Attribute* <administration-guide/configuration-reference/authentication-configuration-settings:id attribute>`.
...
To do this, you can set the :ref:`Login ID Attribute <administration-guide/configuration-reference/authentication-configuration-settings:id attribute>` to whatever you would like users to log in with (e.g. username or email).
...
To address this issue you can set the :ref:`max page size <administration-guide/configuration-reference/authentication-configuration-settings:maximum page size>` in your Mattermost configuration to match the limit on your AD/LDAP server. This will return a sequence of result sets that do not exceed the max page size, rather than returning all results in a single query. A max page size setting of 1500 is recommended.
