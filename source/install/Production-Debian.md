...
2. This can also be set up all on a single server for small teams:
    1. I have a Mattermost instance running on a single Debian Jessie server with 1GB of ram and 30 GB SSD
    2. This has been working in production for ~20 users without issue.
    3. The only difference in the below instructions for this method is to do everything on the same server  
