...
If you are experiencing severely high server and database resource utilization, you may first update the Systems table to stop the job from restarting with ``INSERT INTO Systems VALUES ('fix_crt_channel_unreads', true);``. Then restart the Mttermost application to kill the existing job.
