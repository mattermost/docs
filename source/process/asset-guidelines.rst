...
- Screen size: Provide screenshots that are 20-30% larger than needed - this will make it easier for the screenshot to be included in blog posts, documentation, etc. To do this, close the right-hand sidebar and shrink your screen horizontally to make it narrower, leaving the screen wide enough until the "mobile send button" no longer appears in the bottom-right corner. Make sure not to cut off any sides of the page in the screenshot.
...
- Clarity: Do not include an "unread posts" indicator in the left-hand side, but you can have a few mentions. Also, avoid showing any text with acronyms and abbreviations. Do not show any bugs or defects. Make sure that the area of focus is big enough to read in a blog post, documentation, etc.
...
- Outstanding look: Ensure that the contents of the screenshot shows the Mattermost product in the best light.
   ...
   - Sometimes use custom features (e.g. custom slash commands) and not always the default feature (show off the feature, extendability and customization).
