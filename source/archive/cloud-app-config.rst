...
Mattermost mobile apps can be configured in your EMM solution using AppConfig with the `pre-built apps available on the public app stores (including Google Play and Apple App Store) </mobile/use-prebuilt-mobile-apps.html>`__, or as an `in-house app you compile yourself </mobile/build-custom-mobile-apps.html>`_.
