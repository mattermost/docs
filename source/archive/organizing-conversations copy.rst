...
In addition to this, System Admins can also unarchive channels `via the CLI </manage/command-line-tools.html#mattermost-channel-restore>`_ and Team Admins can unarchive them `via the API <https://api.mattermost.com/#operation/RestoreChannel>`_.
...
Use the **Find channel** option in the channel sidebar, or press CMD/CTRL+K, to find other channels, visit recently reviewed channels, or review `member availability </messaging/setting-your-status-availability.html#setting-your-availability>`__ at a glance. 
