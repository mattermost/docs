...
3. You can restrict who can create incoming webhooks in `System Console > Integrations > Integration Management </configure/configuration-settings.html#enable-incoming-webhooks>`__.
...
To send a message to a different Direct Message channel between two other users, you can specify the channel with the user IDs for the users separated with two underscore (_) symbols. To find the user ID you can `use ``mmctl user search``- see details `here </manage/mmctl-command-line-tool.html#mmctl-user-search>`__.
