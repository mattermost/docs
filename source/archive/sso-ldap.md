...
2. **Configure AD/LDAP by editing `config.json`**
     - Edit `config.json` to enable AD/LDAP based on the [AD/LDAP settings documentation](/configure/configuration-settings.html#ad-ldap). When you log in to Mattermost the first user to log in with valid AD/LDAP credentials will be assigned the System Admin role.
...
2. **Configure AD/LDAP.**
     - Go to **System Console > Authentication > AD/LDAP** and fill in AD/LDAP settings based on the [configuration settings documentation](/configure/configuration-settings.html#ad-ldap).
...
**Note:** If you've made a mistake and lock yourself out of the system somehow, you can [set an existing account to System Administrator using the command line tool](/getting-started/admin-onboarding-tasks.html#common-tasks).
...
2. Scroll down to **Synchronization Interval (minutes)** to specify how often Mattermost accounts synchronize attributes with AD/LDAP. The default setting is 60 minutes. The profile picture attribute is only synchronized when the user logs in.
     ...
     - To configure AD/LDAP synchronization with SAML sign-in, see the [SAML documentation](/onboard/ad-ldap.html).
...
See the [Guest Accounts documentation](/cloud/cloud-guest-accounts.html) for more information about this feature.
...
If the issue persists, try performing a sync with the **User Filter** field blank. If the sync completes in this scenario, then the general syntax was formatted incorrectly. Refer to this [document](/configure/configuration-settings.html#user-filter) for guidance on setting a correct syntax format.
...
2. **User filter:** Set the [user filter](/configure/configuration-settings.html#user-filter) to only select the subset of AD/LDAP users you want to have access to Mattermost. When someone is removed from the selected group, they will be deactivated in Mattermost on the next synchronization.
...
1. Check that the user credentials were entered properly - you should log in with the field set as the [`ID Attribute`](/configure/configuration-settings.html#login-id-attribute).
...
To do this, you can set the [**Login ID Attribute**](/configure/configuration-settings.html#login-id-attribute) to whatever you would like users to log in with (e.g. username or email).
...
To address this issue you can set the [max page size](/configure/configuration-settings.html#maximum-page-size) in your Mattermost configuration to match the limit on your AD/LDAP server. This will return a sequence of result sets that do not exceed the max page size, rather than returning all results in a single query. A max page size setting of 1500 is recommended.
