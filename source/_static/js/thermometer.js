...
jqueryReady(() => {
    ...
    jqueryOn(document.body, 'click', evt => {
        if (evt.target) {
    ...
    jqueryOn(document.body, 'click', evt => {
        if (evt.target) {
            if (evt.target.classList.contains('btn-link')) {
                ...
                document.querySelectorAll('.c-thermometer-modal__container textarea').forEach(elt => {
                    console.debug('set value to empty');
                    elt.value = '';
                });
                document.querySelectorAll('.c-thermometer-modal__counter span').forEach(elt => {
                    elt.innerHTML = '0';
                });
                return;
            }
            let currentValue = '';
            const textareas = document.querySelectorAll('.c-thermometer-modal__container textarea');
            if (textareas && textareas[0]) {
                currentValue = jqueryVal(textareas[0]);
            }
            console.debug(`eventValue=${eventValue}, currentValue=${currentValue}`);
            ...
            document.querySelectorAll('.c-thermometer__emojis').forEach(elt => {
                ...
                elt.classList.add('pointer-events-none');
            });
            document.querySelectorAll('.c-thermometer-modal__container textarea').forEach(elt => {
                console.debug('set value to empty');
                elt.value = '';
