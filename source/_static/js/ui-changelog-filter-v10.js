$(document).ready(function () {
    ...
    // Extract versions from section IDs for v10 format
    const versions = [];
    const sections = [];
    const tocItems = [];
    let uiKeywords = [
    ...
    // Find all release sections based on the v10 heading pattern
    $('section[id^="release-v10"]').each(function () {
        const sectionId = $(this).attr('id');
        const versionMatch = sectionId.match(/release-v(10)[\.-](\d+)/);
    ...
    $('.toc-drawer li').each(function () {
        ...
        if ($link.length) {
            ...
            if (href && href.includes('#')) {
                ...
                // Check if this links to a v10 version section
                const versionMatch = sectionId.match(/release-v(10)[\.-](\d+)/);
                if (versionMatch) {
                    const major = parseInt(versionMatch[1]);
                    const minor = parseInt(versionMatch[2]);
    ...
    // Create enhanced filter controls matching the mockup design
    const filterHTML = `
        <div class="ui-changelog-filters" role="region" aria-label="Changelog Filters">
            <h3>Filter Changelog</h3>
            <p class="help-text">Select version range and change types to see only relevant changelog entries</p>
            
            <div class="filter-row">
                <div class="filter-group">
                    <label for="ui-changelog-from-version">From version:</label>
                    <select id="ui-changelog-from-version" aria-describedby="from-version-help">
                ...
                <div class="filter-group">
                    ...
                    <select id="ui-changelog-to-version" aria-describedby="to-version-help">
                        <option value="all">All versions</option>
                        ${versions.map(v => `<option value="${v}">v${v}</option>`).join('')}
                    </select>
                </div>
            ...
            <div class="filter-row">
                <div class="checkbox-group" role="group" aria-labelledby="change-type-label">
                    ...
                    <div class="checkbox-item">
                        <input type="checkbox" id="show-all-changes" checked aria-describedby="all-changes-help">
                        <label for="show-all-changes">All changes</label>
                    </div>
                    <div class="checkbox-item">
                        <input type="checkbox" id="show-ui-changes" aria-describedby="ui-changes-help">
                        <label for="show-ui-changes">UI changes only</label>
                    </div>
                </div>
            </div>
            
            <div class="filter-buttons">
                <button id="ui-changelog-apply-filter" class="btn-primary" type="button" aria-describedby="apply-help">
                    Apply Filters
                </button>
                <button id="ui-changelog-reset-filter" class="btn-secondary" type="button" aria-describedby="reset-help">
                    Reset
                </button>
            </div>
            
            <div id="ui-filter-status" class="filter-status" role="status" aria-live="polite"></div>
            <div id="ui-filter-error" class="filter-error" role="alert" aria-live="assertive"></div>
            
            <!-- Screen reader helper text -->
            <div class="sr-only">
                <div id="from-version-help">Select the earliest version to include in the filter</div>
                <div id="to-version-help">Select the latest version to include in the filter</div>
                <div id="all-changes-help">Show all types of changes</div>
                <div id="ui-changes-help">Show only user interface related changes</div>
                <div id="apply-help">Apply the selected filters to the changelog</div>
                <div id="reset-help">Clear all filters and show all content</div>
            </div>
        </div>
    `;
    ...
    function updateFilterStatus(fromVersion, toVersion, showAll, showUI) {
        ...
        if (fromVersion === 'all' && toVersion === 'all' && showAll && !showUI) {
            statusEl.text('Showing all changelog entries').addClass('active');
            return;
        }
    ...
    $('#show-all-changes, #show-ui-changes').on('change', function() {
        if ($(this).is(':checked')) {
            if ($(this).attr('id') === 'show-all-changes') {
                $('#show-ui-changes').prop('checked', false);
            } else {
                $('#show-all-changes').prop('checked', false);
            }
        } else {
            // Ensure at least one is always checked
            if (!$('#show-all-changes').is(':checked') && !$('#show-ui-changes').is(':checked')) {
                $('#show-all-changes').prop('checked', true);
            }
        }
    });

    // Apply filter function
    $('#ui-changelog-apply-filter').on('click', function () {
        const fromVersion = $('#ui-changelog-from-version').val();
        const toVersion = $('#ui-changelog-to-version').val();
        const showAllChanges = $('#show-all-changes').is(':checked');
        const showUIChanges = $('#show-ui-changes').is(':checked');
    ...
    // Reset filters
    $('#ui-changelog-reset-filter').on('click', function () {
        $('#ui-changelog-from-version, #ui-changelog-to-version').val('all');
        $('#show-all-changes').prop('checked', true);
        $('#show-ui-changes').prop('checked', false);
        ...
        $('#ui-filter-status, #ui-filter-error').removeClass('active').text('');
    ...
    $('#ui-changelog-from-version, #ui-changelog-to-version').on('change', function() {
        $('#ui-changelog-apply-filter').click();
    });
