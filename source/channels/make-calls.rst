...
Server
^^^^^^^^^^^
- Run Mattermost server on a secure (HTTPs) connection. This is a necessary
  requirement on the client to allow capturing devices (i.e. microphone, screen). See the `Config TLS </install/config-tls-mattermost.html>`_ section for more info.
