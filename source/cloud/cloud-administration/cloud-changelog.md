...
#### User Interface (UI)
 - Changed number of files allowed per post, from 5 to 10.
 - Enabled the 3-dot menu on channels in the new sidebar to move multi-selected groups of channels.
 
#### Administration
 - Bumped the Go version to v1.15.5.
 - Added support for automatic installation and enabling of plugins using feature flags.
 - Added ``webhook create`` endpoints to local mode and the ability to create webhooks for other users.
 - Added a mattermost command to initialize the database.
...
### Bug Fixes
 - Fixed an issue where the permissions of a System Role got deleted when changing the access level to any permission.
 - Fixed an issue where editing a ``/me`` post behaved differently in Webapp and Mobile App.
 - Fixed an issue where the hover state did not span the whole width of the left-hand side on category headers.
 ...
 - Fixed an issue where 15-character team names were truncated when the experimental sidebar was enabled.
 - Fixed an issue where the sidebar menus weren't styled correctly on mobile web view.
 - Fixed an issue where jumping into an archive channel and clicking to jump to recent messages sent the user out of the archived channel.
