...
1. Security related changes were made in 3.9.0 that cause any previously created email invite links, password reset links, and email verification links to no longer work. Email invites that were sent before the upgrade need to be resent if the recipients have not already signed up to your Mattermost instance. Team invite links are not affected and are still valid.
