...
### Breaking Changes
 - The go module path of ``mattermost-server`` was changed to comply with the module version specification. Go projects using ``mattermost-server`` as a library have to change the import path to ``github.com/mattermost/mattermost-server/v5``. You may use ``find . -name "*.go" -type f | xargs sed -i -e 's/github.com\/mattermost\/mattermost-server\//github.com\/mattermost\/mattermost-server\/v5\//g'`` to update the import path.
