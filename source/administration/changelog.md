...
### Breaking Changes
 - The go module path of ``mattermost-server`` was changed to comply with the module version specification. Developers using using ``mattermost-server`` as a library have to change the import path to ``github.com/mattermost/mattermost-server/v5`` when they want to upgrade this dependency to `v5.18`. They may use ``find . -name "*.go" -type f | xargs sed -i -e 's/github.com\/mattermost\/mattermost-server\//github.com\/mattermost\/mattermost-server\/v5\//g'`` to update the import path.
