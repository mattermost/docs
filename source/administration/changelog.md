...
- **v5.23.2, release day TBD**
  - Investigating an issue where users are experiencing high CPU and memory usage after prolonged use of desktop app or webapp. Current workaround is to clear the cache. Workaround for affected desktop app users also includes deleting the AppData folder (Windows: ``%APPDATA%/Mattermost``; Mac: ``~\Library\Application Support\Mattermost``; Linux: ``~\.config\Mattermost``).
