...
- **v5.29.1, release day TBD**
  - Adding ``UnreadMentions`` column to ``ThreadMemberships`` table. Also including additional bug fixes related to Collapsed Reply Threads feature, such as fixing an issue where errors related to ``ThreadMemberships`` appear in the server logs when replying to a user on a thread.
