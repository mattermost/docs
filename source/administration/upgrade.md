...
3. Backup your data
      ...
      3. If you're using local file storage, back up the location where files are stored. You can find where your files are being stored in the `config.json` file under "FileSettings" -> "Directory".
...
5. Restore the state of your server
      ...
      2. If you're using local file storage, restore the data you backed up before running the server. Keep the backup until your sure the upgrade has succeded.
