...
There are two main types of plugins, both of which are supported in Team and Enterprise Editions:
 ...
 - **Client plugins (Beta)**: Allows you to customize the Mattermost user interface, by overriding elements such as the profile popover, channel header or the sidebar. For a sample plugin, see `our zoom plugin <https://github.com/mattermost/mattermost-plugin-zoom>`_.
