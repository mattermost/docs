...
If ``SqlSettings.DataSource`` does not start with ``postgres://`` or ``mysql://`` then you have to add it to the beginning depending which database is in use. Also, if you see ``\u0026`` replace it with ``&``.
