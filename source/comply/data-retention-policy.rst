...
3. Specify a global retention policy for channel messages and Direct Messages by selecting a **Channel & direct message retention** option from the dropdown, then specify how long to keep those messages. When a time is specified, messages and file attachments older than the duration you set will be deleted. The minimum retention period is one day.
