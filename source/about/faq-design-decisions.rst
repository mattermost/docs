...
When using username-password authentication, especially with Entra ID, there's the possibility of usernames being email addresses, Mattermost username, Entra ID username and ID, or other Entra ID attributes and our design principle intends to help end users understand whether their login error came from having the wrong password or the wrong email/username.
