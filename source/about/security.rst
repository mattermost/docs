...
Security in Mattermost software is a constant endeavor by developers, IT administrators, and security researchers accountable for deploying the software in their organizations.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   - Mattermost can run entirely behind your firewall as a single Linux binary, as a Docker container, or on your Kubernetes cluster with MySQL or PostgreSQL as database. Remote access can be enabled through the `VPN clients on PC and mobile devices <https://docs.mattermost.com/deploy/deployment-overview.html#behind-a-vpn>`__ so that Mattermost can be accessed outside your private network.
   - Mattermost mobile apps can be deployed to an `internal Enterprise App Store <https://docs.mattermost.com/deploy/deploy-mobile-apps-using-emm-provider.html>`__using source code available for Mattermost mobile apps and push notification service. 
...
~~~~~~~~~~~~~~~~~~~~~~
   ...
   - Mattermost supports TLS encryption using AES-256 with 2048-bit RSA on all data transmissions between Mattermost client applications and the Mattermost server across both LAN and the internet.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   - Use an `antivirus plugin <https://github.com/mattermost/mattermost-plugin-antivirus>`__ to scan for viruses before uploading a file to Mattermost. Supports `ClamAV anti-virus software <https://www.clamav.net/>`__ across browsers, Desktop App, and Mobile Apps.
   - Custom `data retention policies on messages and file uploads <https://docs.mattermost.com/comply/data-retention-policy.html>`__ are available (E20).A daily data deletion job can be scheduled that will delete messages from the database and user interface, and file uploads from local file storage or Amazon S3, after they have exceeded the specified retention period.
...
~~~~~~~~~~~~~~~~~~~~~~~~~~
   ...
   - Session length, session cache, and idle timeout can be `configured according to your internal policies <https://docs.mattermost.com/configure/configuration-settings.html#session-lengths>`__, automatically forcing a user to re-login after a specified duration.
...
Mattermost undergoes extensive penetration testing, security reviews, and `security updates <https://mattermost.com/security-updates/>`__. You can find further details and previous discussions `in our GitHub thread <https://github.com/mattermost/platform/issues/4321#issuecomment-258832013>`__. In addition, the following policies are provided:
...
- `Mattermost Security Updates Disclosures <https://mattermost.com/security-updates/>`__
   - A summary of security updates is made based on past and on-going security analysis and penetration testing.
...
FINRA reviews a firm’s ability to protect the confidentiality, integrity, and availability of sensitive customer information. also reviewing each firm’s compliance with SEC regulations which includes:

- Regulation `S-P (17 CFR §248.30) <https://www.ecfr.gov/cgi-bin/text-idx?SID=226b4b62d8bf25d29cc88df5039cddde&mc=true&node=se17.4.248_130&rgn=div8>`__, that mandates firms to adopt written policies and procedures for protecting customer's information against cyber-attacks and other forms of unauthorized access.

- Regulation `S-ID (17 CFR §248.201-202) <https://www.ecfr.gov/cgi-bin/text-idx?SID=5621786ec1a831400e4b64f3e92198bd&mc=true&node=pt17.4.248&rgn=div5#sp17.4.248.c>`__,which outlines a firm duties regarding the detection, prevention, and mitigation of identity theft.
...
- **Multi-layered disaster recovery:** High Availability configuration, automated data backup, and enterprise information archiving integration to prevent data loss and recovery from disaster.
