...
- `Mattermost Notification Health metrics <https://grafana.com/grafana/dashboards/21305-mattermost-notification-health>`_, which can be used to track different types of notifications sent from Mattermost. Accessing Mattermost Notification Health requires the feature flag ``EnableNotificationMetrics`` to be set to ``true``. System admins can :ref:`disable notification metrics data collection <configure/environment-configuration-settings:enable notification metrics>` through the System Console.
- `Mattermost WebApp Performance Metrics <https://grafana.com/grafana/dashboards/XXXXX>`_, which contains detailed metrics for client-side performance, including x, y, and z. Accessing Mattermost WebApp Performance Metrics requires the feature flag ``EnableClientMetrics`` to be set to ``true``.
