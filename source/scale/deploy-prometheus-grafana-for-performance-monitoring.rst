...
- `Mattermost WebApp Performance Metrics <https://grafana.com/grafana/dashboards/XXXXX>`_, which contains detailed metrics for client-side performance, including x, y, and z. Accessing Mattermost WebApp Performance Metrics requires the feature flag ``ClientMetrics`` to be set to ``true``.
