...
- **System Console**: User management screen in **System Console > Users** allows system admins to deactiveate users with email/password login. See the :ref:`Deactivate users <configure/user-management-configuration-settings:deactivate users>` documentation for details.
...
Starting from Mattermost server v10.10, when a user is deactivated, their availability status is automatically set to offline to ensure accurate status reporting and improve the user experience.
