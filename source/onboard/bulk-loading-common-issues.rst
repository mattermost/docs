...
If you are using a Mattermost version prior to v5.17, extended `password requirements </configure/configuration-settings.html#password>`__ also need to be disabled (to remove requirements of uppercase, special characters, numbers, and the password length of 5). Any passwords that do not meet the requirements will cause the user import to fail.
