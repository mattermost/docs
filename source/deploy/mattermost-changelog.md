...
#### User Interface (UI)
 ...
 - Added a new user preference to disable webapp prefetching via **Settings > Advanced > Allow Mattermost to prefetch channel posts**. This setting should only be enabled if users are experiencing performance issues. Client Performance Debugging needs to be enabled in the System Console in order for this setting to appear.
