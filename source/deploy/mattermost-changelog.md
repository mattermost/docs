...
#### User Interface (UI)
 ...
 - Added a new user preference to disable webapp prefetching via **Settings > Advanced > Allow Mattermost to prefetch channel posts**. This setting should only be enabled if users are experiencing performance issues. You must enable **Client Performance Debugging** in the System Console by going to **Environment > Developer** in order for this setting to appear.
