...
3. Navigate to the release page for `version v6.2.3 <https://github.com/mattermost/desktop/releases/latest>`__ and download the appropriate installer for your version of Windows (32-bit vs. 64-bit).

4. Download the `source.zip <https://github.com/mattermost/desktop/archive/v6.2.3.zip>`__ file as well to extract group policy files.
...
**Command Prompt:** ``msiexec /i mattermost-desktop-v6.2.3-x64.msi /qn``

**PowerShell:** ``Start-Process -FilePath "$env:systemroot\system32\msiexec.exe" -ArgumentList '/i mattermost-desktop-v6.2.3-x64.msi /qn'``

.. note::
   - Replace ``<version>`` with the actual version number (e.g., ``v6.2.3``).
...
- **Command Prompt:** ``msiexec /i mattermost-desktop-v6.2.3-x64.msi APPLICATIONFOLDER="<install directory>"``
- **PowerShell:** ``Start-Process -FilePath "$env:systemroot\system32\msiexec.exe" -ArgumentList '/i mattermost-desktop-v6.2.3-x64.msi APPLICATIONFOLDER="<install directory>"'``
