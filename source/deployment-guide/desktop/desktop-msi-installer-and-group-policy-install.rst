...
This page provides guidance on installing the desktop app MSI and use Group Policies in Windows for Mattermost Enterprise or Professional. The MSI installer package can be downloaded `here <https://github.com/mattermost/desktop/releases/tag/v6.0.1>`_.
...
3. Navigate to the release page for `version v6.0.1 <https://github.com/mattermost/desktop/releases/tag/v6.0.1>`__ and download the appropriate installer for your version of Windows (32-bit vs. 64-bit).

4. Download the `source.zip <https://github.com/mattermost/desktop/archive/v6.0.1.zip>`__ file as well to extract group policy files.
...
1. Browse to the folder the above files were downloaded to and unzip the ``desktop-6.0.1.zip`` file in place.
...
2. Navigate to the unzipped ``desktop-6.0.1\resources\windows\gpo`` folder and copy the contents.
...
**Command Prompt:** ``msiexec /i mattermost-desktop-v6.0.1-x64.msi /qn``

**PowerShell:** ``Start-Process -FilePath "$env:systemroot\system32\msiexec.exe" -ArgumentList '/i mattermost-desktop-v6.0.1-x64.msi /qn'``
...
**Command Prompt:** ``msiexec /i mattermost-desktop-v6.0.1-x64.msi ALLUSERS=1``

**PowerShell:** ``Start-Process -FilePath "$env:systemroot\system32\msiexec.exe" -ArgumentList '/i mattermost-desktop-v6.0.1-x64.msi ALLUSERS=1'``

.. note::
   - Installing the MSI for all users disables automatic updates for the desktop app on Windows.
   - To disable automatic updates on a per-user basis, use the ``DISABLEAUTOUPDATE`` parameter: ``msiexec /i mattermost-desktop-v6.0.1-x64.msi DISABLEAUTOUPDATE=1``
...
**Command Prompt:** ``msiexec /i mattermost-desktop-v6.0.1-x64.msi APPLICATIONFOLDER="<install directory>"``

**PowerShell:** ``Start-Process -FilePath "$env:systemroot\system32\msiexec.exe" -ArgumentList '/i mattermost-desktop-v6.0.1-x64.msi APPLICATIONFOLDER="<install directory>"'``
