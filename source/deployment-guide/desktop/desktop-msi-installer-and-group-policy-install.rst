...
3. Navigate to the release page for `version v6.2.1 <https://github.com/mattermost/desktop/releases/latest>`__ and download the appropriate installer for your version of Windows (32-bit vs. 64-bit).

4. Download the `source.zip <https://github.com/mattermost/desktop/archive/v6.2.1.zip>`__ file as well to extract group policy files.
...
1. Browse to the folder the above files were downloaded to and unzip the ``desktop-6.2.1.zip`` file in place.
...
2. Navigate to the unzipped ``desktop-6.2.1\resources\windows\gpo`` folder and copy the contents.
...
**Command Prompt:** ``msiexec /i mattermost-desktop-v6.2.1-x64.msi /qn``

**PowerShell:** ``Start-Process -FilePath "$env:systemroot\system32\msiexec.exe" -ArgumentList '/i mattermost-desktop-v6.2.1-x64.msi /qn'``

.. note::
   - Replace ``<version>`` with the actual version number (e.g., ``v6.2.1``).
...
- **Command Prompt:** ``msiexec /i mattermost-desktop-v6.2.1-x64.msi APPLICATIONFOLDER="<install directory>"``
- **PowerShell:** ``Start-Process -FilePath "$env:systemroot\system32\msiexec.exe" -ArgumentList '/i mattermost-desktop-v6.2.1-x64.msi APPLICATIONFOLDER="<install directory>"'``
