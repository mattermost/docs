...
.. important::
   ...
   - If you are planning to use the calling functionality in **Preview Mode** on a non-local environment, you should ensure that the server is running on a secure (HTTPs) connection and that the :ref:`network requirements <administration-guide/configure/calls-deployment:network requirements>` to run calls are met.
