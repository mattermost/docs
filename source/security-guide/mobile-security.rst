...
Administrators can configure the maximum age of cached content and how long data persists after a device goes offline. A server-initiated wipe triggered by access revocation removes all data, including credentials, even before the device reconnects. Learn more about :ref:`Mobile Ephemeral Mode <deployment-guide/mobile/mobile-security-features:mobile ephemeral mode>`.
