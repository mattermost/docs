...
# example command: pipenv run python scripts/dump_doctree.py build/doctrees/.../xxxx.doctree
...
def truncate(text, size=20, ellipse="...", inline=True, strip=True):
    if inline:
        text = text.replace("\n", " ")
    if strip:
        text = text.strip()
...
def truncate_path(text, level=2, ellipse=".../"):
    import os
...
def starttag(node, quoteattr=None):
    if quoteattr is None:
        quoteattr = nodes.pseudo_quoteattr
...
def pformat(node, indent="  ", level=0):
