...
Gitaly is configured from [global settings][globals]. Please see documentation
[configuring Gitaly there](../../globals.md#configure-gitaly-settings).
...
The `authToken` attribute for shell has two sub keys:
- `secret` defines the name of the kubernetes `Secret` to pull from
