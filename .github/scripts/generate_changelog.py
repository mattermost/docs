...
1.  **Section structure:** Use `###` for top-level sections and `####` for subsections. Only include sections that have relevant content — do not output empty sections. Do NOT add horizontal rules or line separators between sections. Do NOT add a blank line between any heading and the content that immediately follows it (whether a paragraph, bullet list, or any other element).
...
5.  **Markdown formatting:** Indent each bullet point with two spaces (e.g., `  - item`). Ensure correct and clean Markdown syntax throughout. Do not insert horizontal rules (`---`) or any other separators between sections. Do not add a blank line between any heading and the content that immediately follows it.
...
def main():
    ...
    # Mattermost uses a bare sentence (not a bullet point) in this section.
    ...
    if go_version:
        ...
        formatted_go = extract_previous_go_version(changelog_path) or "(see previous release)"
