...
class TabHtmlTransform(SphinxPostTransform):
    ...
    def run(self):
        ...
        for tab_container in all_tab_containers:
            logger.info(f"{LOG_PREFIX} run ({self.env.docname}) processing TabContainer {tab_container.tab_identifier()}")
    ...
    def _collect_tab_containers(self, node: nodes.Node, tab_containers: List[TabContainer]):
        """
        ...
        if isinstance(node, TabContainer):
            ...
            logger.info(f"{LOG_PREFIX} _collect_tab_containers ({self.env.docname}) found TabContainer: {node.tab_identifier()}")
    ...
    def _process_one_node(self, node: TabContainer):
        ...
        last_tab_set = self.stack[-1]
        last_tab = last_tab_set[-1]
        
        logger.info(f"{LOG_PREFIX} _process_one_node ({self.env.docname}|{node.tab_identifier()}) comparing with last tab: {last_tab.tab_identifier()}")
        
        # Check if this is a child of the last tab (nested tab)
        if self._is_direct_child(node, last_tab):
            logger.info(f"{LOG_PREFIX} _process_one_node ({self.env.docname}|{node.tab_identifier()}) is child of {last_tab.tab_identifier()}; start new nested tab set")
            self.stack.append([node])
            return

        # Check if this should be grouped with the current tab set
        # For top-level tabs: same parent and not marked as new-set
        if (node.parent == last_tab.parent and 
            self._get_tab_level(node) == self._get_tab_level(last_tab) and
            not node["new_set"]):
            logger.info(f"{LOG_PREFIX} _process_one_node ({self.env.docname}|{node.tab_identifier()}) same level and parent as {last_tab.tab_identifier()}; add to current tab set")
        ...
        node_level = self._get_tab_level(node)
        last_level = self._get_tab_level(last_tab)
        
        if node_level < last_level:
            # This is at a higher level (less nested), close deeper sets
            logger.info(f"{LOG_PREFIX} _process_one_node ({self.env.docname}|{node.tab_identifier()}) higher level ({node_level} vs {last_level}); close deeper sets")
            ...
            if (self.stack and 
                node.parent == self.stack[-1][-1].parent and
                self._get_tab_level(node) == self._get_tab_level(self.stack[-1][-1]) and
                not node["new_set"]):
                logger.info(f"{LOG_PREFIX} _process_one_node ({self.env.docname}|{node.tab_identifier()}) adding to parent level tab set")
                self.stack[-1].append(node)
                return

        # Start a new tab set
        logger.info(f"{LOG_PREFIX} _process_one_node ({self.env.docname}|{node.tab_identifier()}) starting new tab set")
        self.stack.append([node])

    def _determine_node_relationship(self, node: TabContainer) -> dict:
        """
        ...
        for i, tab_set in enumerate(reversed(self.stack)):
            stack_index = len(self.stack) - 1 - i  # Convert reversed index to actual stack index
            logger.info(f"{LOG_PREFIX} _determine_node_relationship ({self.env.docname}|{node.tab_identifier()}) checking tab set at stack level {stack_index}")
            ...
            for tab_node in tab_set:
                if self._is_direct_child(node, tab_node):
                    logger.info(f"{LOG_PREFIX} _determine_node_relationship ({self.env.docname}|{node.tab_identifier()}) is child of {tab_node.tab_identifier()}")
            ...
            if node_level == 0:
                ...
                for tab_node in tab_set:
                    if (self._get_tab_level(tab_node) == 0 and 
                        node.parent == tab_node.parent and
                        not node["new_set"]):
                        logger.info(f"{LOG_PREFIX} _determine_node_relationship ({self.env.docname}|{node.tab_identifier()}) is top-level sibling of {tab_node.tab_identifier()}")
                        return {"type": "sibling", "tab_set": tab_set, "sibling_node": tab_node}

            # Check if this node is a sibling of any node in this tab set (for nested tabs)
            sibling_result = self._check_sibling_relationship(node, tab_set)
            if sibling_result["is_sibling"]:
                logger.info(f"{LOG_PREFIX} _determine_node_relationship ({self.env.docname}|{node.tab_identifier()}) is nested sibling at level {stack_index}")
                return {"type": "sibling", "tab_set": tab_set, "sibling_node": sibling_result["sibling_node"]}

        # No relationship found
        logger.info(f"{LOG_PREFIX} _determine_node_relationship ({self.env.docname}|{node.tab_identifier()}) no relationship found")
        return {"type": "unrelated"}

    def _check_sibling_relationship(self, node: TabContainer, tab_set: List[TabContainer]) -> dict:
        """
        ...
        for tab_node in tab_set:
            if self._is_sibling_at_same_level(node, tab_node):
                logger.info(f"{LOG_PREFIX} _check_sibling_relationship ({self.env.docname}|{node.tab_identifier()}) is sibling of {tab_node.tab_identifier()}")
                return {"is_sibling": True, "sibling_node": tab_node}
        return {"is_sibling": False}

    def _is_sibling_at_same_level(self, node: TabContainer, potential_sibling: TabContainer) -> bool:
        """
        Check if node is a sibling of potential_sibling at the same hierarchical level.
        """
        # Quick check: same parent and same level
        if (node.parent == potential_sibling.parent and 
            self._get_tab_level(node) == self._get_tab_level(potential_sibling)):
            logger.info(f"{LOG_PREFIX} _is_sibling_at_same_level ({self.env.docname}) {node.tab_identifier()} is sibling of {potential_sibling.tab_identifier()}")
            return True
        return False
    ...
    def _is_direct_child(self, node: TabContainer, potential_parent: TabContainer) -> bool:
        ...
        content_container = potential_parent.children[1]
